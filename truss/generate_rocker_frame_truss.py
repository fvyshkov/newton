...
угол→центр радиали, угол→вершина ноги башни) — под каждую приваривается
place_clamp с прорезью вверх/наружу для доступа к болту.

  1. Rocker_corner_node.stl ×2 — рядовые углы 30° и 150°. Узел в
     polar(R_CORNER, ang, FRAME_Z) сводит 5 хомутов-гнёзд:
       • 2 хорды к двум другим углам (горизонт),
       • 1 радиаль к центру рамы (горизонт),
       • 2 ноги башни ВВЕРХ-НАРУЖУ к ОБЕИМ вершинам (±170,0,530) —
         каждая вершина раскосится всеми тремя углами → жёсткая башня.
     Снизу — плоское дно z≈61 (обрезка) с карманом под PTFE-скользяк:
     узел едет по дорожке кольца z=60. Центральная бобышка кармана держит
     мост потолка и несёт M3-саморез PTFE-подушки.
...
STANDOFF = 14.0         # вынос нижнего торца хомута от центра узла, мм
BORE_D = TC.BORE_D      # Ø бора ≈ 21.94 (труба Ø22)
CLAMP_OD = TC.CLAMP_OD  # воротник ≈ 46.6 ⟂ прорези
SLIT_UP = (0.0, 0.0, 1.0)   # прорезь смотрит вверх/наружу → болт доступен сбоку

# --- Литой узел ---
HUB_R = 26.0            # радиус центральной сферы-ступицы узла, мм
NODE_BOT_Z = 61.0       # плоское дно узла (обрезка) — 1мм над дорожкой (60)
...
def socket(node, endpoint, slit=SLIT_UP):
    """Хомут-гнездо: ось бора вдоль трубы node→endpoint, нижний торец вынесен
    на STANDOFF от центра узла (тело трубы входит снаружи, до ступицы)."""
    node = np.asarray(node, float)
    d = np.asarray(endpoint, float) - node
    d /= np.linalg.norm(d)
    origin = node + d * STANDOFF
    return TC.place_clamp(CLAMP_H_SOCK, origin, tube_dir=d, slit_dir=slit)


def crop_bottom(part, z_bot):
    """Срезать всё ниже z_bot плоским дном (большой куб-резак)."""
    cutter = box(4000, 4000, 4000, T=tr(0, 0, z_bot - 2000))
    return part.difference(cutter)
...
def corner_node(ang, drive=False):
    """Литой угловой узел: сфера-ступица + 5 хомутов (2 хорды, 1 радиаль,
    2 ноги башни к обеим вершинам) + плоское дно z=61 с карманом PTFE.
    drive=True → задний узел 270°: добавляет пад 4×M4 под Rocker_swing_base и
    радиальный проём под шестерню к венцу."""
    ...
    for _tag, ep in eps:                               # приварить все гнёзда
        part = part.union(socket(node, ep))
    ...
    part = crop_bottom(part, NODE_BOT_Z)               # плоское дно z=61
...
def center_node():
    ...
    # 3 радиальных гнезда к углам
    for ang in CORNER_ANGLES:
        part = part.union(socket(node, corner_xyz(ang)))
