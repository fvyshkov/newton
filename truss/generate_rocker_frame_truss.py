...
GLIDE_RAD = 16.0        # лапа ПОПЕРЁК дорожки (радиально), мм
GLIDE_TAN = 20.0        # лапа ВДОЛЬ дорожки (тангенциально), мм = размер тефлон-пятака
GLIDE_R_CTR = 208.5     # РАДИУС центра лапы (сдвинут внутрь от R_CORNER=210): чтобы
                        # с учётом ДУГИ все 4 угла легли в тело венца R200..217:
                        # наружн.угол √(216.5²+10²)=216.7<217, внутр.√(200.5²+10²)=200.7>200
...
def corner_node(ang, drive=False, drive_window=True, drive_pad=True,
    ...
    # центр лапы — на GLIDE_R_CTR вдоль радиуса узла (сдвинут внутрь, чтобы углы
    # с учётом ДУГИ легли в тело венца), не под самим узлом (R_CORNER=210)
    gx = GLIDE_R_CTR * math.cos(math.radians(ang))
    gy = GLIDE_R_CTR * math.sin(math.radians(ang))
    ...
    # узкая сторона (RAD) поперёк дорожки, вписана в тело венца R200..217
