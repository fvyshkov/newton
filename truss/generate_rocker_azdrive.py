...
PIN_FACE = 10.0         # высота шестерни = высоте зубьев венца (10мм). БЕЗ ступицы:
                        # старая ступица Ø20×8 сверху упиралась в шкив 60T —
                        # 20мм шестерня не влезала между фанерой (низ) и шкивом (верх).
...
HUB_D = 20.0            # Ø ступицы (не используется — ступица убрана)
HUB_H = 0.0             # ступицы НЕТ (была 8мм — не влезала под шкив 60T)
SET_HOLE = 3.2          # Ø радиального отверстия под M3-фиксатор
SET_Z = PIN_FACE / 2.0                     # ось фиксатора по центру тела, z=5
...
# ============================ 2. ШЕСТЕРНЯ 16T ============================
def rocker_pinion():
    """16T модуль 2, ПЛОСКАЯ шестерня z=0..10 (= высота зубьев венца), БЕЗ
    ступицы. Бор Ø8.4 насквозь. Радиальный M3-фиксатор по центру тела (z=5)
    давит на вал через гайку в пазу-кармане (гайка вставляется сверху; идея
    ...
    Ставится любой стороной; зубья на всю высоту → любой ориентацией в венце."""
    ...
    # Ступицы НЕТ: фиксатор и гайка садятся прямо в тело зубьев
    # (ядро R13.5 > борт бора R4.2 → стенка ~9мм, хватает на M3 + карман гайки).
    total_h = PIN_FACE
    bore = cyl(PIN_BORE / 2, total_h + 2, T=tr(0, 0, total_h / 2))

    # Радиальное отверстие фиксатора вдоль +X (ось по Z → повернуть на Y);
    # длина насквозь — от бора наружу за вершину зуба (R18).
    ss_len = PIN_TIP_R + 8.0
    setscrew = cyl(SET_HOLE / 2, ss_len, seg=32)
    setscrew.apply_transform(trimesh.transformations.rotation_matrix(
        math.pi / 2, [0, 1, 0]))
    setscrew.apply_translation([ss_len / 2 - 6.0, 0, SET_Z])
...
def rocker_mesh_test():
    ...
    # Основание: от дуги до колонн П-вилки (шире в Y под две колонны)
    base = box(58, 70, TEST_BASE_T, T=tr(242, 0, -TEST_BASE_T / 2))
    part = base.union(arc)
